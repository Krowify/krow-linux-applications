// Matugen template -- input for [templates.quickshell-overview] in
// config.toml. Generates ~/.config/quickshell/overview/common/
// Appearance.colors.qml from the current wallpaper. Ported directly from
// Shanu-Kumawat/quickshell-overview's own template (a loose file at the
// module root, apparently their own build input for the same file) --
// verbatim, since it's already shaped exactly like every other Matugen
// template in this repo (role names match m3primary/m3onPrimary/etc., the
// QML properties the shipped default in dotfiles/quickshell-overview/
// common/Appearance.colors.qml also declares).
import QtQuick
